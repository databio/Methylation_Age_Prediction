...
setwd("/Users/anant")
data <- fread("HPC_11_S26751_epilog.bed")
DT = data.table(data)
head(DT)
...
new_DT
write.table(new_DT, file = "2_26_HPC_11.cov", row.names = FALSE, col.names = FALSE, sep = "\t" )

#Up until here, just processing the initial .bed file with collapsing

DT2 <- fread('2_26_HPC_11.cov')
DT2
DT2[V1 == 10]
dim(DT2)
c <- 1 
...
write.table(new_DT_2, file = "2_26_bothsides_HPC11.cov", row.names = FALSE, col.names = FALSE, sep = "\t")
